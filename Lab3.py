...
def parse_with_pecmd(file_path, output_dir):
    try:
        # Run PECmd command
        subprocess.run(["PECmd.exe", "-f", file_path, "-d", output_dir], check=True)
...
def parse_with_recmd(file_path, output_dir):
    try:
        # Run RECmd command
        subprocess.run(["RECmd.exe", "-d", file_path, "-o", output_dir], check=True)
...
def main():
    ...
    sam_file = os.path.join(script_dir, "sam")
    system_file = os.path.join(script_dir, "system")
